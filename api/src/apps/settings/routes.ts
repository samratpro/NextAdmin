...
export default async function settingsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Querystring: { type: 'logo' | 'favicon' } }>(
    ...
    async (request, reply) => {
      ...
      const ALLOWED_EXTS = new Set(['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.ico']);
      const ext = path.extname(data.filename).toLowerCase() || '.png';
      if (!ALLOWED_EXTS.has(ext)) {
        return reply.status(400).send({ error: `File type "${ext}" is not allowed. Accepted: jpg, png, gif, webp, svg, ico` });
      }
