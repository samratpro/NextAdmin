...
export default async function blogRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/public/posts', {
    ...
    let filters: Record<string, any> = { published: true };

    // Filtering by category if provided
    if (category) {
        const cat = await Category.objects.get<Category>({ slug: category });
        if (cat) {
            filters.categoryId = cat.id as number;
        }
    }

    const total = await BlogPost.objects.filter(filters).count();
    const paginatedPosts = await BlogPost.objects.filter<BlogPost>(filters)
