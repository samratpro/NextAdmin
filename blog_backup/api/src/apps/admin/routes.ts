...
export default async function adminRoutes(fastify: FastifyInstance) {
    ...
    fastify.get('/api/admin/models/:modelName/data', {
        ...
        if (!request.user?.isSuperuser) {
            const hasPermission = await permissionService.hasModelPermission(
                request.user!.userId, 'view', modelName
            );
            if (!hasPermission) {
                reply.code(403).send({ error: 'Permission denied' });
                return;
        ...
        const searchFields: string[] = metadata.adminOptions.searchFields || [];

        // Build WHERE clause for search (LIKE on each searchField)
        let whereClause = '';
        ...
        if (search && search.trim() && searchFields.length > 0) {
            ...
            searchFields.forEach(() => whereParams.push(term));
        }

        // Total count (with search applied)
        ...
        const rows = await db.all<Record<string, any>>(
            `SELECT * FROM ${tableName}${whereClause} ORDER BY ${orderBy} ${orderDirection} LIMIT ${limit} OFFSET ${offset}`,
            whereParams
        );

        // Strip excluded fields
        const safeInstances = rows.map(row => {
            const d = { ...row };
            for (const key of excluded) delete d[key];
            return d;
        });
    ...
    fastify.post('/api/admin/models/:modelName/data', {
        ...
        // Auto-populate createdByAdminId if field exists and was not provided (still a Field object)
        const createdByVal = (instance as any).createdByAdminId;
        if (createdByVal === undefined || createdByVal === null || typeof createdByVal === 'object') {
            (instance as any).createdByAdminId = request.user!.userId;
        }
