...
export default async function authRoutes(fastify: FastifyInstance) {
  ...
  // Setup Password (Onboarding)
  fastify.post('/auth/setup-password', {
    preHandler: requireAuth,
    schema: {
      tags: ['Authentication'],
      description: 'Setup initial password for auto-created accounts',
      body: {
        type: 'object',
  ...
  fastify.patch('/auth/me', {
    ...
    try {
      const { username, firstName, lastName } = request.body as any;
      const user = await authService.updateProfile(request.user!.userId, { username, firstName, lastName });
