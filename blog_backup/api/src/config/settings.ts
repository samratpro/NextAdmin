...
// Always load the API env file from the api root so running from a different
// working directory does not silently switch configuration/DB.
const envPath = path.resolve(__dirname, '../../.env');
config({ path: process.env.DOTENV_CONFIG_PATH || envPath });
...
const envSchema = z.object({
  ...
  ENCRYPTION_KEY: z.string().default('00000000000000000000000000000000'),
});
...
const settings: AppSettings = {
  ...
  database: {
    ...
    path: path.isAbsolute(env.DB_PATH) ? env.DB_PATH : path.resolve(__dirname, '../../', env.DB_PATH),
  ...
  cors: {
    origin: env.CORS_ORIGIN?.split(',') || [`http://localhost:${env.PORT}`, 'http://localhost:7000', 'http://localhost:5000', 'http://localhost:3000'],
    credentials: true,
  },
